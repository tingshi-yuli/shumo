...
\subsection{问题提出}
题目要求依次回答四个问题：问题1在附录2物性下给出1800秒内的温度和含水率分布；问题2在附录3物性下给出前3小时及全过程结果；问题3在固定半径条件下确定全域含水率低于0.15 kg/kg的报告时刻；问题4在附录4物性和附件2收缩半径下重新确定该时刻，并给出移动边界下的空间结果。
\subsection{任务与数据}
...
题目四问使用同一类状态变量但采用递进条件：问题1用于获得预热响应；问题2建立附录3全过程轨迹；问题3在该轨迹上定位全域事件；问题4重新输入附录4物性和收缩几何，不能将问题3结果按半径比例直接缩放。
...
\section{模型建立和求解}
\subsection{问题一：预热阶段}
\subsubsection{问题一模型：控制方程、边界与附录2物性}
...
\end{equation}
\subsubsection{共享离散方法：区间通量与表面加密}
...
\subsection{问题二：全过程温湿传递}
问题二从初始状态直接使用附录3物性。上一节给出的控制方程和守恒离散保持不变，本节仅替换物性函数并保存逐秒状态，以满足全过程输出要求。
\subsubsection{问题二结果}
...
\subsection{问题三：全域达标事件}
问题三继承问题二的附录3物性和固定半径轨迹，在同一状态序列上施加全域最大含水率阈值，并对首次跨越事件进行局部重算。
\subsubsection{问题三结果}
...
\subsection{问题四：收缩条件下的热湿传递}
问题四切换附录4经验物性，并用附件2半径构造移动材料坐标；固定物理距离超过当前半径时不外推。
\subsubsection{问题四结果}
